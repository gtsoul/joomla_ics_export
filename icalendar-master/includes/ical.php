	...
	if($data != ""){
		...
		foreach($lines as $line) {
			...
			$linecount++;
				...
				$datanode = new ZCiCalDataNode("PRODID:-//excaliburyvelines.fr//CEY ICS Export 1.0//FR");

				$this->curnode->data[$datanode->getName()] = $datanode;
				$datanode = new ZCiCalDataNode("X-WR-CALNAME:Excalibur Yvelines");
				$this->curnode->data[$datanode->getName()] = $datanode;
				$datanode = new ZCiCalDataNode("X-WR-TIMEZONE:Europe/Paris");
				$this->curnode->data[$datanode->getName()] = $datanode;
...
static function fromSqlDateTime($datetime = ""){
	...
	if(strlen($datetime) > 10)
		return sprintf('%04d%02d%02dT%02d%02d%02d',substr($datetime,0,4),substr($datetime,5,2),substr($datetime,8,2),
			substr($datetime,11,2),substr($datetime,14,2),substr($datetime,17,2)) . 'Z';
	else
		return sprintf('%04d%02d%02d',substr($datetime,0,4),substr($datetime,5,2),substr($datetime,8,2)) . 'Z';
}
